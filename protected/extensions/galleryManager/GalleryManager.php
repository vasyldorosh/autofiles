...
class GalleryManager extends CWidget
{
    ...
    public function run()
    {
        ...
        $opts = array(
            //'hasName:' => $this->gallery->title ? true : false,
            //'hasDesc:' => $this->gallery->content ? true : false,
			'hasName:' => true,
            'hasDesc:' => true,
        ...
        $cls = "GalleryEditor ";
        if (isset($this->gallery->title) && !($this->gallery->title)) $cls .= 'no-name';

        if (isset($this->gallery->content) && !($this->gallery->content)) {
            $cls .= (($cls != ' ') ? '-' : '') . 'no-desc';
        }
