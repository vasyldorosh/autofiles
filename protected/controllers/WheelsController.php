...
class WheelsController extends Controller
{
	...
	public function actionModelYear($makeAlias, $modelAlias, $year)
	{
		...
		$this->pageTitle = str_replace(array('[make]', '[model]', '[year]'), array($make['title'], $model['title'], $modelYear['year']), SiteConfig::getInstance()->getValue('seo_wheels_model_year_title'));
		$this->meta_keywords = str_replace(array('[make]', '[model]', '[year]'), array($make['title'], $model['title'], $modelYear['year']), SiteConfig::getInstance()->getValue('seo_wheels_model_year_meta_keywords'));
		$this->meta_description = str_replace(array('[make]', '[model]', '[year]'), array($make['title'], $model['title'], $modelYear['year']), SiteConfig::getInstance()->getValue('seo_wheels_model_year_meta_description'));		
		$header_text_block = str_replace(array('[make]', '[model]', '[year]'), array($make['title'], $model['title'], $modelYear['year']), SiteConfig::getInstance()->getValue('wheels_model_year_header_text_block'));		
		...
		$this->breadcrumbs = array(
			'/' => 'Home',
			'/wheels.html' => 'Wheels',
			'/wheels'.$make['url'] => array('anchor'=>$make['title'], 'title'=>$make['title'] . ' wheels'),
			'/wheels'.$model['url'] => array('anchor'=>$model['title'], 'title'=>$make['title'] . ' ' . $model['title'] . ' wheels'),
		...
		$wheels = AutoModelYear::getWheels($modelYear['id']);	
		//d($wheels);
	...
	public function actionDiameter($value)
	{
		...
		$this->pageTitle = str_replace(array('[diameter]'), array($value), SiteConfig::getInstance()->getValue('seo_wheels_diameter_title'));
		$this->meta_keywords = str_replace(array('[diameter]'), array($value), SiteConfig::getInstance()->getValue('seo_wheels_diameter_meta_keywords'));
		$this->meta_description = str_replace(array('[diameter]'), array($value), SiteConfig::getInstance()->getValue('seo_wheels_diameter_meta_description'));		
		$header_text_block = str_replace(array('[diameter]'), array($value), SiteConfig::getInstance()->getValue('wheels_diameter_header_text_block'));		
		...
		$this->render('diameter', array(
			'wheels' => Tire::getItemsByRimDiameterNonRunflat($model['id']),
			'model' => $model,
			'header_text_block' => $header_text_block,
		));	
	}	
	...
	public function actionSize($vehicle_class, $section_width, $aspect_ratio, $rim_diameter)
	{
		...
		$wheels = Tire::getItemsByAttributes($attributes);
		...
		$this->pageTitle = str_replace($replaceFrom, $replaceTo, SiteConfig::getInstance()->getValue('seo_wheels_size_title'));
		$this->meta_keywords = str_replace($replaceFrom, $replaceTo, SiteConfig::getInstance()->getValue('seo_wheels_size_meta_keywords'));
		$this->meta_description = str_replace($replaceFrom, $replaceTo, SiteConfig::getInstance()->getValue('seo_wheels_size_meta_description'));		
	...
	public function actionSizeMake($makeAlias, $vehicle_class, $section_width, $aspect_ratio, $rim_diameter)
	{
		...
		$this->pageTitle = str_replace($replaceFrom, $replaceTo, SiteConfig::getInstance()->getValue('seo_wheels_size_make_title'));
		$this->meta_keywords = str_replace($replaceFrom, $replaceTo, SiteConfig::getInstance()->getValue('seo_wheels_size_make_meta_keywords'));
		$this->meta_description = str_replace($replaceFrom, $replaceTo, SiteConfig::getInstance()->getValue('seo_wheels_size_make_meta_description'));		
		$header_text_block = str_replace($replaceFrom, $replaceTo, SiteConfig::getInstance()->getValue('wheels_size_make_header_text_block'));		
