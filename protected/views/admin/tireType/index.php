<?php $this->pageTitle = Yii::app()->name.' - ' . Yii::t('admin', 'Tire Types');?>

<div class="container inner-page white img-rounded">
    <div class="page-header">
        <h3><?=Yii::t('admin', 'Tire Types')?></h3>
    </div>
	...
	<div class="button">
		<?php if (Access::is('tire.type.create')):?>
			<? $this->widget("bootstrap.widgets.TbButtonGroup", array(
				'type' => 'success',
				'buttons' => array(
					array('label' => Yii::t('admin', 'Create'), 'url' => Yii::app()->createUrl("admin/tireType/create"), 'icon' => 'file white'),
				),
			))?>
		<?php endif;?>
    ...
    <div class="grid">
		...
		$columns = array(
				...
				array(
					...
					'name' => 'value',
					'htmlOptions' => array(
						'access' => 'tire.type.update', 
					),	
				),			
				'rank',				
            );
			
		if (Access::is('tire.type.update') || Access::is('tire.type.delete')) {
			$template = array();
			if (Access::is('tire.type.update')) $template[] = '{update}';
			if (Access::is('tire.type.delete')) $template[] = '{delete}';
