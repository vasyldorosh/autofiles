<?php $this->pageTitle = Yii::app()->name.' - ' . Yii::t('admin', 'Tire Load Indexs');?>

<div class="container inner-page white img-rounded">
    <div class="page-header">
        <h3><?=Yii::t('admin', 'Tire Load Indexs')?></h3>
    </div>
	...
	<div class="button">
		<?php if (Access::is('tire.load_index.create')):?>
			<? $this->widget("bootstrap.widgets.TbButtonGroup", array(
				'type' => 'success',
				'buttons' => array(
					array('label' => Yii::t('admin', 'Create'), 'url' => Yii::app()->createUrl("admin/tireLoadIndex/create"), 'icon' => 'file white'),
				),
			))?>
		<?php endif;?>
    ...
    <div class="grid">
		...
		if (Access::is('tire.load_index.update') || Access::is('tire.load_index.delete')) {
			$template = array();
			if (Access::is('tire.load_index.update')) $template[] = '{update}';
			if (Access::is('tire.load_index.delete')) $template[] = '{delete}';
