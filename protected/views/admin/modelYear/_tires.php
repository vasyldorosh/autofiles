	...
	<div class="control-group">
		<?php $model->post_tires_related = $model->getPost_tires_related()?>
			...
			<?php 	
				...
				foreach (AutoModelYear::getListYears($model->model_id) as $item): if($id==$model->id){continue;}?>
				<label><input type="checkbox" name="AutoModelYear[post_tires_related][]" value="<?=$item['id']?>" <?=in_array($id, $model->post_tires_related)?'checked="checked"':''?>> <span <?=($modelYearTires==$item['tires'])?'style="font-weight: bold;"':''?>><?=$item['year']?></span> <?=(count($item['tires'])>0)?'('.count($item['tires']).')':''?></label>
			<?php endforeach;?>
			</div>
	</div>
	<?php endif;?>