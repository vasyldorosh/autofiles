<?php $this->pageTitle = Yii::app()->name.' - ' . Yii::t('admin', 'Reviews');?>

<div class="container inner-page white img-rounded">
    <div class="page-header">
        <h3><?=Yii::t('admin', 'Reviews')?></h3>
    </div>
	...
	<div class="button">
		<?php if (Access::is('modelYear.review.create')):?>
			<? $this->widget("bootstrap.widgets.TbButtonGroup", array(
				'type' => 'success',
				'buttons' => array(
					array('label' => Yii::t('admin', 'Create'), 'url' => Yii::app()->createUrl("admin/review/create"), 'icon' => 'file white'),
				),
			))?>
		<?php endif;?>
    ...
    <div class="grid">
		...
		$columns = array(
				...
				array(
					...
					'name' => 'title',
					'htmlOptions' => array(
						'access' => 'modelYear.review.update', 
					),	
				),			
				'alias',				
            );
			
		if (Access::is('modelYear.review.update') || Access::is('modelYear.review.delete')) {
			$template = array();
			if (Access::is('modelYear.review.update')) $template[] = '{update}';
			if (Access::is('modelYear.review.delete')) $template[] = '{delete}';
