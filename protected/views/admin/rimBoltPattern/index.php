<?php $this->pageTitle = Yii::app()->name.' - ' . Yii::t('admin', 'Rim Bolt Pattern');?>

<div class="container inner-page white img-rounded">
    <div class="page-header">
        <h3><?=Yii::t('admin', 'Rim Bolt Pattern')?></h3>
    </div>
	...
	<div class="button">
		<?php if (Access::is('tire.rim.bolt_pattern.create')):?>
			<? $this->widget("bootstrap.widgets.TbButtonGroup", array(
				'type' => 'success',
				'buttons' => array(
					array('label' => Yii::t('admin', 'Create'), 'url' => Yii::app()->createUrl("admin/rimBoltPattern/create"), 'icon' => 'file white'),
				),
			))?>
		<?php endif;?>
    ...
    <div class="grid">
		...
		if (Access::is('tire.rim.bolt_pattern.update') || Access::is('tire.rim.bolt_pattern.delete')) {
			$template = array();
			if (Access::is('tire.rim.bolt_pattern.update')) $template[] = '{update}';
			if (Access::is('tire.rim.bolt_pattern.delete')) $template[] = '{delete}';
