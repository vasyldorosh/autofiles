<?php $this->pageTitle = Yii::app()->name.' - ' . Yii::t('admin', 'Tire Vehicle Classs');?>

<div class="container inner-page white img-rounded">
    <div class="page-header">
        <h3><?=Yii::t('admin', 'Tire Vehicle Classs')?></h3>
    </div>
	...
	<div class="button">
		<?php if (Access::is('tire.vehicle_class.create')):?>
			<? $this->widget("bootstrap.widgets.TbButtonGroup", array(
				'type' => 'success',
				'buttons' => array(
					array('label' => Yii::t('admin', 'Create'), 'url' => Yii::app()->createUrl("admin/tireVehicleClass/create"), 'icon' => 'file white'),
				),
			))?>
		<?php endif;?>
    ...
    <div class="grid">
		...
		$columns = array(
				...
				array(
					...
					'name' => 'title',
					'htmlOptions' => array(
						'access' => 'tire.vehicle_class.update', 
					),	
				),			
				'rank',				
            );
			
		if (Access::is('tire.vehicle_class.update') || Access::is('tire.vehicle_class.delete')) {
			$template = array();
			if (Access::is('tire.vehicle_class.update')) $template[] = '{update}';
			if (Access::is('tire.vehicle_class.delete')) $template[] = '{delete}';
