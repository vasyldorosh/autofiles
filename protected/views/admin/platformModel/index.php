<?php $this->pageTitle = Yii::app()->name.' - ' . Yii::t('admin', 'Platform Model');?>

<div class="container inner-page white img-rounded">
    <div class="page-header">
        <h3><?=Yii::t('admin', 'Platform Model')?></h3>
    </div>
	...
	<div class="button">
		<?php if (Access::is('modelYear.platform.create')):?>
			<? $this->widget("bootstrap.widgets.TbButtonGroup", array(
				'type' => 'success',
				'buttons' => array(
					array('label' => Yii::t('admin', 'Create'), 'url' => Yii::app()->createUrl("admin/platformModel/create"), 'icon' => 'file white'),
				),
			))?>
		<?php endif;?>
    ...
    <div class="grid">
		...
		if (Access::is('modelYear.platform.update') || Access::is('modelYear.platform.delete')) {
			$template = array();
			if (Access::is('modelYear.platform.update')) $template[] = '{update}';
			if (Access::is('modelYear.platform.delete')) $template[] = '{delete}';
