...
/*	$('a.go-card').click(function() {
			...
			location = '/index.php?r=site/findMap&q='+q+'&address='+address+'&category='+cat+'&startPrice='+startPrice+'&endPrice='+endPrice;
		});*/
</script>
...
<script src="<?php echo Yii::app()->request->baseUrl; ?>/js/home-actions.js"></script>
...
<script>
    ...
    function moveMap(pos, needCenter){
		...
		var wasOf = SOffset;
        $.ajax({
            url: "<?php echo Yii::app()->createUrl('site/getBallonsActions', array('count'=>10, 'offset'=> ''))?>"+SOffset,
            data: {
                maps: false,
                lat: pos.l,
                long: pos.d,
            ...
            success: function(data){
                console.log(data);
                // todo переписать на нормальную перезагрузку
                $(".page").removeClass("page-show").addClass("page-hide");
				if (wasOf == 0) {
					$('.salons-holder .action').parent().remove();
				...
				for (var i in data.data) {
					var salon = data.data[i];
					for (var j in salon.offers) {
						var offer = salon.offers[j];
						...
						var o = {
							...
							salon: salon.name,
							salonId: salon.id,
							podarok: offer.present,
							metros: salon.metros,
							address: salon.address,
							old_cost: offer.old_price,
							cost: offer.price,
							rubles: offer.discount,
							type: Math.random() > 0.5?'present':'star',
							rest: "3 дня"
						};
						addAction($('.salons-holder'),o, SOffset, "cat");
						SOffset++;

					}
				}
                if (SOffset%2==1) $('.salons-holder').append('</tr>'); //если нечет - закончим ряд
                ...
                if (len === 0) {
                    ...
                    /*$('.found').html("Мы нашли для Вас <span>"+len+'</span> '+end);*/
                }
                for(var i in data.data) {
                    var d = data.data[i];
                    var ball = new ymaps.Placemark([d.l, d.d], {
                        balloonContentBody:
                            '<a href="<?= Yii::app()->createUrl('site/salon')?>&id='+d.id+'"><b>'+d.name+'</b></a><br>'
                                 +d.address+"<br>"
                                +d.phone+"<br>"
                                +d.work_time,
                        draggable: true,
				...
				if (wasOf+data.data.length >= data.total) {
					$('.load-actions').hide();
				} else {
					$('.load-actions').show();
				}
...
<? include dirname(__FILE__).'/includes/search_form.php' ?>
      ...
      <!-- BOF Salons -->
      <div class="well salons-holder">
        <p class="lead  text-center page_header">Акции</p>
        <div class='found'></div>
      </div>
      <!-- EOF Salons -->
      <br />
